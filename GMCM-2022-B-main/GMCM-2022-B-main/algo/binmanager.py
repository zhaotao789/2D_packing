...
class BinManager:
    ...
    def items_sort(self): 
        ...
        elif self.sorting_heuristic == 'DESCSS':     #按小的降序，这样可以优先排序大物体
            key = lambda el: el.width if el.width < el.height else el.height
            self.items.sort(key=key, reverse=True)
    ...
    def _bin_best_fit(self, item: item.Item) -> bool:
        ...
        scores = []
        for binn in self.bins:   #判断是否有合适的stage存放当前订单
            #s, _, _ = binn._find_best_score(item)[:3]
            s, _, _ = binn._find_best_score(item)  #binn是Guillotine类型
            if s is not None:
                scores.append((s, binn))
        if scores:
            _, best_bin = min(scores, key=lambda x: x[0])   #让剩余的面积最小化
            return best_bin.insert(item)
        new_bin = self._bin_factory()   #新建一个stage
        new_bin.insert(item, self.heuristic)   #将剩余区域切割
