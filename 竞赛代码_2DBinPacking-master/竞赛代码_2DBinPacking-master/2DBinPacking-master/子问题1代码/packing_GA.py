...
for f_num in file_num:
    data_file = r'D:\aliyun\2D装箱\竞赛代码_2DBinPacking-master\2DBinPacking-master\子问题1-数据集A/dataA{}.csv'.format(f_num)
    if dataA is None:
...
df_result.to_csv(r"D:\aliyun\2D装箱\竞赛代码_2DBinPacking-master\2DBinPacking-master\子问题1-数据集A\dataA_test.csv",encoding='utf-8-sig')
...
def main():
    ...
    for bin_num,cur_bin in enumerate(best_bins):
        ...
        if df_result is None:
            ...
            df_result = pd.concat((df_result,cur_result),axis=0)
        # view(cur_result,bin_num)
    df_result.to_csv(r'D:\aliyun\2D装箱\竞赛代码_2DBinPacking-master\2DBinPacking-master\子问题1-数据集A/dataA_GA.csv',encoding='utf-8-sig',index=None)
