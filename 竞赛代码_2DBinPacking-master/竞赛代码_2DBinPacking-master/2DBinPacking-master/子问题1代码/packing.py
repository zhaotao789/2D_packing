...
def packing(data):
    ...
    while item_ids:
        ...
        mini_height = min(data[data['item_id'].isin(item_ids)]['item_length'])   #全局最小宽高
...
def view(bin_result,bin_num=0):
    ...
    for i in range(len(bin_result)):
        ...
        rect = plt.Rectangle((x,y),width,height,
                             ...
                             edgecolor='red')
        ax.add_patch(rect)
        ax.text(x+width/2,y+(height)/2,'{}'.format(bin_result.loc[i]['产品id']),rotation=90)
    plt.xlim(0,1220)
    plt.ylim(0,2440)
    plt.title('原片{}'.format(bin_num))
    plt.savefig('D:/aliyun/2D装箱\竞赛代码_2DBinPacking-master/2DBinPacking-master/result/原片{}.jpg'.format(bin_num))
    #plt.show()
    plt.close()



dataA = pd.read_csv(r'D:\aliyun\2D装箱\竞赛代码_2DBinPacking-master\2DBinPacking-master\子问题1-数据集A/dataA1.csv',header=0)

# 按item_length 排序
# dataA['min_length'] =dataA.apply(lambda el: el.item_length if el.item_length < el.item_width else el.item_width,axis=1)
# dataA.sort_values(by=['min_length'],ascending=False,inplace=True)
